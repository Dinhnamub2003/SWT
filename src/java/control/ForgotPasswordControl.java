...
public class ForgotPasswordControl extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			DAO dal = new DAO();
			Account account = dal.checkAccountExistByUsernameAndEmail(username, emailAddress);
			if(account == null) {
				request.setAttribute("error", "Email hoac username sai!");
			}
			if(account != null) {
				Email email =new Email();
				email.setFrom("quyendinhnam.ub2003@gmail.com");
				email.setFromPassword("oxni ckxx mota ysea");
